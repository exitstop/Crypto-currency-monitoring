...
func GetPriceKucoin(coin string) (map[string]interface{}, error){
	...
	if err != nil{ return nil, errors.New(nameFunction + "() -> Get()") }
	json, err := Connect(resp)
	if err != nil{ return nil, errors.New(nameFunction + "() -> Connect()") }
	if mp, ok := json["data"].(map[string]interface{}); ok{
		return 	mp, nil
	}else{
		return 	nil, errors.New("GetPriceKucoin() Not foun key json['data']")
	}
}
